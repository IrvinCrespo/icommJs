...
module.exports = class BaseBuilder{
 ...
 select(fields){
    var selectf = "*"
    if(fields)selectf = this.table+"."+this.primaryKey+","+fields.join(",")+"";
...
//Regresar modelo :V
 find(id){
    ...
    this.query = ("SELECT * FROM "+this.table).concat(this.query);
    console.log("LA QUERY: ",this.query);
    var self = this;
    return new Promise((result,reject)=>{
        connect(function(err,connection){
            connection.escape(self.query);
...
//Manipulating data
 ...
 buildJoins(){
    ...
    this.query = this.query.replace("FROM "+table,"FROM "+this.table+strjoin);
    console.log("QUERY JOIN: ",this.query);
    return this.query;
}
